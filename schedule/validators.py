...
class AppointmentValidator:
    def __init__(self, data, user=None, errors=None, ErrorClass=None):
    ...
    def clean(self, *args, **kwargs):
        ...
        if Appointment.objects.filter(date=date, time=time, user=self.user).first():
            self.errors['date'].append(
                _('An appointment at this date and time '
                  'has already been booked by you!')
            )
        ...
        unavailable_times = Appointment.objects.filter(
            date=date_str, user=self.user).values_list('time', flat=True)
