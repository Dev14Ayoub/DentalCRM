...
class UserUpdateView(View):
    ...
    def get_updated_data(self, user, data):
        for key in data:
            if isinstance(data[key], list):
                data[key] = data[key][0]
        # Extract phone_number directly from data instead of nested profile dict
    ...
    def post(self, request):
        ...
        form = UpdateForm(form_data, FILES, instance=request.user)
        if form.is_valid():
            user = form.save()
            # Debug logging for phone_number
            phone_number = form.cleaned_data.get('phone_number', '')
            print(f"Debug: phone_number from form.cleaned_data: '{phone_number}'")
            print(f"Debug: current profile phone_number: '{user.profile.phone_number}'")
            ...
            print("UpdateForm errors:", form.errors)
        context = {
            'form': form,
            'form_action': reverse('users:update'),
        }
