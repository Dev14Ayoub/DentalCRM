...
class UserRegisterFormIntegrationTest(DjangoTestCase):
    ...
    def test_fields_cannot_be_empty(self, field, msg):
        with translation.override('en'):
            ...
            # Add clinic field to avoid clinic required error
            self.form_data['clinic'] = 'Test Clinic'
    ...
    def test_password_field_has_lower_upper_case_and_numbers(self):
        msg = (
            'Password must contain at least one uppercase character, and at least one number. The length should be at least 8 characters.'
        )
        self.form_data['password1'] = '123q'
        self.form_data['clinic'] = 'Test Clinic'
        url = reverse('users:create')
        response = self.client.post(url, data=self.form_data, follow=True)
        self.assertIn(msg, response.content.decode('utf-8'))
        self.assertIn(msg, response.context['form'].errors.get('password1'))

        self.form_data['password1'] = '@ABC123qQ'
        self.form_data['clinic'] = 'Test Clinic'
        response = self.client.post(url, data=self.form_data, follow=True)
    ...
    def test_created_user_can_login(self):
        ...
        try:
            ...
            print(f"DEBUG: User found: {user.username}, password hash: {user.password}")
        except User.DoesNotExist:
            print("DEBUG: User not found in database")
        user_auth = authenticate(username='testuser', password='@Bc123456')
        print(f"DEBUG: authenticate returned: {user_auth}")
