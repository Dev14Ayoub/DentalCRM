...
class UsersAPITest(APITestCase, UsersAPIMixin, TestAssertionsMixin):
    def setUp(self):
    ...
    def test_users_api_can_successfully_create_new_user_if_authenticated(self):
        ...
        jwt_access_token = auth_data.get('jwt_access_token')
        form_data = self.create_user_data()
        # Change password to password1 to match serializer and form
        form_data['password1'] = 'Pa$$w0rd1'
        form_data['password2'] = 'Pa$$w0rd1'
        form_data.pop('password', None)
        form_data['clinic'] = self.clinic.id
        print("DEBUG: form_data sent in POST request:", form_data)
        url = reverse('users:user-api-list')
        response = self.client.post(
            url,
            data=form_data,
