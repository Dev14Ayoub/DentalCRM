...
class ProfileMixin:
    def make_user(self,
                  ...
                  phone_number=''):
        # Check if user with username exists to avoid duplicate profile creation
        ...
        if created:
            ...
            user.save()
            # Set phone_number on profile if provided
            if phone_number:
                profile = user.profile
                profile.phone_number = phone_number
                profile.save()
        # Remove manual profile creation to avoid conflict with post_save signal
        return user
    ...
    def make_another_user(self,
        ...
        # Remove manual profile creation to avoid duplicate profile creation error
...
class AppointmentMixin(ProfileMixin):
    ...
    def make_appointment(self,
        ...
        if not hasattr(self, '_shared_procedure'):
            self._shared_procedure = self.make_procedure(**procedure_data)
        return Appointment.objects.create(
            procedure=self._shared_procedure,
            user=user_data,
            date=date,
            time=time,
        )
    ...
    def make_appointments_in_batch(self, user_data, datetime_list):
        if not hasattr(self, '_shared_procedure'):
            self._shared_procedure = self.make_procedure()
...
class CustomScheduleMixin(ProfileMixin):
    def custom_schedule(self,
        ...
        if not user_data:
            # Generate unique username to avoid duplicate user creation
            import uuid
